...
st.markdown('<h1 style="color: Red;">YouTube Data Harvesting and Warehousing</h1>', unsafe_allow_html=True)
#st.write('<h1 style="color: blue;">Process flow</h1>', unsafe_allow_html=True)
...
list_options = ['Retrieving data',
                'Migrate data', 'Query Data']
option = st.selectbox('Select One', list_options)

#Retrieving data from the YouTube API and store to mongoDB
if option=='Retrieving data':
     if st.button('Channel_id'):
          st.write("View channel home page - Right click on the screen - select 'view page source' - search'?channel id'")
     channelid=st.text_input('Enter channel id')
     if st.button('Enter'):
        extract_and_upload(channelid)
